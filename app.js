...
function taskTwo() {
  ...
  document.getElementById('answer-two').innerHTML = numbersAsString;
}
...
function taskFive() {
  ...
  document.getElementById('answer-five').innerHTML = 
    `<p><strong>Ätligt:</strong> ${eatable} <br> 
    <strong>Skräp:</strong> ${trash}</p>`;
}
...
function taskSix() {
  ...
  `<p><strong>Över 30:</strong> ${overThirty} <br>
    <strong>Under 30:</strong> ${underThirty} <br>
    <strong>Gift:</strong> ${married} <br>
    <strong>Ogift:</strong> ${notMarried}</p>`;
}
...
function taskNine() {
  ...
  const allAnswerContainers = document.querySelector(".wrapper")
    .querySelectorAll(".answer-container");
  if(hour >= 17 ) {
    for(let i = 0; i < allAnswerContainers.length; i++) {
    allAnswerContainers[i].style.backgroundColor = 'blue';
    }
  } else {
    for(let i = 0; i < allAnswerContainers.length; i++) {
      allAnswerContainers[i].style.backgroundColor = 'red';
    }
  }
}
